...
// //       const titleElement = $(element).find("a.WKTcLC, div.WKTcLC");
// //       const title = titleElement.length ? titleElement.text().trim() : "";

// //       const priceElement = $(element).find("div.Nx9bqj");
...
// //       const ratingElement = $(element).find('div[class*="XQDdHH"]');
// //       const rating = ratingElement.length ? ratingElement.text().trim() : "";

// //       const linkElement = $(element).find(
// //         "a.VJA3rP, a.s1Q9rs, a._2UzuFa, div._4rR01T"
// //       );
// //       const link = linkElement.length
// //         ? "https://www.flipkart.com" + linkElement.attr("href")
// //         : "";
  ...
  try {
    ...
    const products = await page.evaluate(() => {
      ...
      productElements.forEach((element) => {
        var titleElement = element.querySelector("a.WKTcLC, div.WKTcLC , a.wjcEIp");
        ...
        const linkElement = element.querySelector("a.VJA3rP, a.s1Q9rs, a._2UzuFa, div._4rR01T");
...
app.get('/compare', async (req, res) => {
  ...
  try {
    ...
    await amazonPage.goto(`https://www.amazon.in/s?k=${input}`, { waitUntil: 'load', timeout: 0 });
    ...
    const image2 = $('div.CXW8mj._3nMexc img._396cs4._2amPTt._3qGmMb').attr('srcset')?.split(' ')[0];
    ...
    $1('.s-result-item').each((index, element) => {
      ...
      const amazonPrice = $1(element).find('.a-price-whole').first().text().trim();
      const amazonRating = $1(element).find('.a-icon-alt').first().text().trim();
    ...
    let a_title = null, a_price = null, a_rating = null, a_link = null;

    for (let i = 0; i < amazonProducts.length; i++) {
      const productx = amazonProducts[i];
      const flipkartTitleFirstWord = product[0].title.split(' ')[0].toLowerCase();
